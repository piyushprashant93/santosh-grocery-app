  ...
  if (error) {
    return (
      <div className="flex flex-col h-full items-center justify-center min-h-[400px] text-center">
        ...
        <button onClick={fetchData} className="px-5 py-2.5 bg-orange-500 text-theme-text rounded-lg font-medium hover:bg-orange-600 transition">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="px-4 py-2 bg-theme-surface border border-gray-200 text-gray-700 font-medium rounded-lg shadow-sm hover:bg-gray-50 transition disabled:opacity-70 flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Total Users */}
        <div className="bg-theme-surface p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between h-[140px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Platform Breakdown */}
          <div className="bg-theme-surface rounded-[13px] border border-theme-border p-6 text-theme-text flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-[9px] flex-1">
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="flex items-start gap-2 text-theme-muted text-[13.5px] font-medium leading-[18px]">
                  <UserPlus size={16} className="mt-0.5" />
                  <span>New<br/>Users<br/>(Today)</span>
                </div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{(analyticsData?.newUsers?.[0]?.count || 0).toLocaleString()}</span>
              </div>
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="text-theme-muted text-[13.5px] font-medium leading-[18px]">Retailers</div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{getRoleCount('retailer').toLocaleString()}</span>
              </div>
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="text-theme-muted text-[13.5px] font-medium leading-[18px]">Restaurants</div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{getRoleCount('restaurant').toLocaleString()}</span>
              </div>
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="text-theme-muted text-[13.5px] font-medium leading-[18px]">Products</div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{((dData as any).totalProducts || 4500).toLocaleString()}</span>
              </div>
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="flex items-center gap-2 text-theme-muted text-[13.5px] font-medium leading-[18px]">
                  <Utensils size={16} />
                  <span>Food Items</span>
                </div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{((dData as any).totalFoodItems || 2800).toLocaleString()}</span>
              </div>
              <div className="bg-[#1D293D]/50 border-[0.67px] border-[#314158]/50 rounded-[9px] p-[14px] flex flex-col justify-between min-h-[100px]">
                <div className="text-theme-muted text-[13.5px] font-medium leading-[18px]">Categories</div>
                <span className="text-[26px] font-bold text-theme-text mt-2 leading-none">{((dData as any).totalCategories || 45).toLocaleString()}</span>
              </div>
            </div>
          </div>

          {/* Pending Approvals */}
          <div className="bg-gradient-to-br from-[#FE9A00] to-[#F54900] rounded-[13px] shadow-[0_4px_6px_-4px_rgba(0,0,0,0.1),0_10px_15px_-3px_rgba(0,0,0,0.1)] p-[27px] text-theme-text flex-1 flex flex-col justify-center gap-[9px] relative overflow-hidden">
            <h2 className="text-[22px] font-bold" style={{ fontFamily: 'serif' }}>Pending Approvals</h2>
            <p className="text-theme-text/90 text-[15px] leading-relaxed relative z-10">
              You have {dData.pendingProducts || 12} new vendor requests and {dData.pendingRestaurants || 5} restaurant applications waiting.
            </p>
            <button className="bg-theme-surface text-[#F54900] text-[15px] font-semibold py-3 px-4 rounded-[8px] w-full shadow-sm hover:bg-orange-50 transition relative z-10 mt-2">
              Review Applications
            </button>
          </div>
